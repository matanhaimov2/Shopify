...
const generateAccessToken = (user) => {
  return jwt.sign({ userId: user._id, username: user.username, cartInfo: user.cartInfo, isAdmin: user.isAdmin }, JWT_SECRET, { expiresIn: '10m' });
};


router.post("/register", async (req, res) => {
  try {
    const { username, password, cartInfo, isAdmin } = req.body;
